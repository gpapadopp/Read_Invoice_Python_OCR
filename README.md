...
4.	After typing the above, then the program will execute its algorithm, where it works as follows:
    ...
    h.  Once the parallel lists of data in a row have been created, the program stores these parallel lists in an Access database. It aims to store the customer's VAT number, the invoice sending e-mail, as well as the page number corresponding to this customer. After doing this, the program performs a iteration to scan all the data from the database to the clients. Depending on whether the client has one or more pages, the query in the database will return one or more entries to the database. It then checks if the entries returned by the database are one, or multiple. If there is only one entry in the database for this customer, then they will send an e-mail, with only one PDF, ie the PDF corresponding to this customer's invoice page. If there are multiple entries in the database for this client, then the program will read all the pages it needs to send to this client, and will create a PDF with all the invoice pages. Whether the sending page is one or more, the program will pass to the send_email function the path of the PDF that should be attached to the sending e-mail to the client.
...
Η παρούσα εφαρμογή δημιουργήθηκε για την αποστολή τιμολογίων και την διευκόλυνση του χρήστη στην διαχείριση του πελατολογίου του, καθώς και των τιμολογίων των πελατών του.
...
4.	Αφού πληκτρολογήσει τα παραπάνω, τότε το πρόγραμμα θα εκτελέσει τον αλγόριθμο του, όπου λειτουργεί ως εξής:
    ...
    h.  Αφού έχουν δημιουργηθεί οι παράλληλες λίστες με τα δεδομένα σε μια σειρά, τότε το πρόγραμμα αποθηκεύει τις παράλληλες αυτές λίστες σε μια βάση δεδομένων της Access. Αυτή, έχει ως στόχο, να αποθηκεύσει το ΑΦΜ του πελάτη, το e-mail αποστολής του τιμολογίου, καθώς και τον αριθμό της σελίδας που αντιστοιχεί με αυτόν τον πελάτη. Αφού το κάνει αυτό, το πρόγραμμα εκτελεί μια επανάληψη, για να σαρώσει όλα τα στοιχεία από τη βάση δεδομένων, ως προς τους πελάτες. Ανάλογα αν ο πελάτης έχει μια, ή και πολλές σελίδες, το ερώτημα στη βάση δεδομένων, θα επιστρέψει μια, ή πολλές εγγραφές στη βάση δεδομένων. Έπειτα, ελέγχεται αν η εγγραφές που έχει επιστρέψει η βάση δεδομένων, είναι μια, ή πολλές. Αν υπάρχει μόνο μια εγγραφή στη βάση δεδομένων, για αυτόν τον πελάτη, τότε θα στείλει ένα e-mail, μόνο με ένα PDF, δηλαδή το PDF που αντιστοιχεί στη σελίδα τιμολογίου αυτού του πελάτη. Αν υπάρχουν πολλαπλές εγγραφές στη βάση δεδομένων, για αυτόν τον πελάτη, τότε το πρόγραμμα θα διαβάσει όλες τις σελίδες που πρέπει να στείλει σε αυτόν τον πελάτη, και θα δημιουργήσει ένα PDF με όλες τις σελίδες των τιμολογίων. Είτε η σελίδα αποστολής είναι μια, είτε είναι πιο πολλές, το πρόγραμμα θα παραδώσει στη συνάρτηση send_email το path του PDF που θα πρέπει να επισυναφθεί στο e-mail αποστολής στον πελάτη.
